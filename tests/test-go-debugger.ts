/**
 * Test script for the Go debugger using Delve
 */
...
async function main() {
  console.log('=== Go Debugger Test (Delve) ===\n');
  ...
  try {
    ...
    // Set a breakpoint at line 17 (result := calculate(a, b))
    console.log('\n2. Setting breakpoint at line 17...');
    const bp = await manager.setBreakpoint(sessionId, { file: SOURCE_PATH, line: 17 });
    ...
    const startResult = await manager.startDebugging(sessionId, {
      ...
      cwd: path.dirname(SOURCE_PATH),  // Set working directory explicitly
      stopOnEntry: false
    });
