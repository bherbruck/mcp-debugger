...
export class RustAdapter {
    ...
    buildLaunchConfig(params, _executablePath) {
        // Determine if this is a source file or a binary
        const isSourceFile = params.scriptPath.endsWith('.rs');
        const isCargoProject = !isSourceFile;
        let config;
        if (isCargoProject) {
            // Debug via cargo
            config = {
                type: 'lldb',
                request: 'launch',
                name: 'MCP Debug Rust (Cargo)',
                cargo: {
                    args: ['build', '--bin', path.basename(params.scriptPath)],
                    filter: {
                        kind: 'bin'
                    }
                },
            ...
            // Debug a compiled binary or source file
            config = {
                type: 'lldb',
                request: 'launch',
                name: 'MCP Debug Rust',
